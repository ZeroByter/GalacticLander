...
public class TrailerLevelLoader : MonoBehaviour {
    public static TrailerLevelLoader Singletron;
    ...
    private void Awake() {
        if (string.IsNullOrEmpty(GameLevelDirectory) && SceneManager.GetActiveScene().name != "Trailer") {
        ...
        //reading level
        FileStream file = File.Open(Application.persistentDataPath + "/Level Editor/Levels/trailer.level", FileMode.Open);
        file.Position = 0;
        ...
        if (levelData != null) {
            ...
            foreach (LevelObject obj in levelData.levelData) {
                if(obj.GetType() == typeof(LevelEntity) || obj.isEntity) {
                    ...
                    {
                        newEntity.GetComponent<SpriteRenderer>().enabled = false;
                        newEntity.transform.GetChild(0).gameObject.SetActive(false);
                    }
