...
public class MissileController : MonoBehaviour {
    ...
    private MissileLauncherController GetClosestMissileLauncher() {
        ...
        foreach (GameObject entityObject in GameObject.FindGameObjectsWithTag("Entity")) { //loop through all entities
    ...
    private void Awake() {
        ...
        if (NetworkingManager.CurrentLobbyValid) { //if we are in a lobby it means our missile launcher variable is not set, we have to fix that
            MissileLauncherController closestLauncher = GetClosestMissileLauncher(); //get closest launcher

            if(closestLauncher != null) { //if it's valid
                //setup vars
    ...
    private void Update() {
        ...
        Vector3 dir = closestShipPosition - missileTransform.position;
        ...
        missileTransform.rotation = Quaternion.Lerp(missileTransform.rotation, rot, missileTurnSpeed);
        missileTransform.eulerAngles = new Vector3(0, 0, missileTransform.eulerAngles.z);
    ...
    private void FixedUpdate() {
        if (NetworkingManager.CurrentLobbyValid) { //if we are in a lobby
            if (nObject == null || !nObject.IsMine()) { //if we are not the player who instantiated this missile launcher (the lobby owner) then halt/exit method
                return;
            }
        }

        if (missileLauncher != null) {
            if (Vector2.Distance(missileLauncher.transform.position, transform.position) > Constants.MissileKillDistance) { //if we got out of range of our missile launcher
                Destroy(gameObject);
            }
        }
    ...
    private GameObject GetPlayerParentObject(GameObject go) {
        if(go.transform.parent != null && go.transform.parent.gameObject.tag == "Player") { //if parent of object is a player object
            return GetPlayerParentObject(go.transform.parent.gameObject);
        } else { //if parent is not parent object
            return go;
        }
    }
